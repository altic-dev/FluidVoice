...
# copy the per-user bearer token for clients:
TOKEN=$(defaults read com.FluidApp.app LocalAPIAuthToken)
```
...
- Every route requires the per-user bearer token generated by FluidVoice. Read it with
  `defaults read com.FluidApp.app LocalAPIAuthToken` and send `Authorization: Bearer <token>`.
  This prevents processes in other macOS login sessions from using FluidVoice's file access or data.
