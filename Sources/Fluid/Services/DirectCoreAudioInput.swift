...
nonisolated enum DirectCoreAudioDeviceSelection: Equatable {
    ...
    /// Bind `uid` when it is present, otherwise the macOS default input. FluidVoice-only mode pins a
    /// microphone for this app alone but does not require it: refusing to record when the pinned mic
    /// is unplugged is worse for the user than recording the default and announcing the swap. Note
    /// this only chooses which device to capture — it never moves the system default input.
